...
3.  **服务启动**（可选）：执行您配置的 [启动命令](08-start-and-ready-commands.md)。
4.  **就绪检查**：执行 [就绪命令](08-start-and-ready-commands.md) 确认环境已完全可用。
...
### 用户与目录
关于沙箱内的默认权限与工作目录，请参阅 [用户与工作目录](03-user-and-work-dir.md)。
